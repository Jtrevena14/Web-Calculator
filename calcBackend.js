...
function addNums(a){ //Number is added to display string (for whole comp), and current number just for the current number not including signs
    displ+=a;
    currNum+=a;
    document.getElementById("labelbott").innerText=displ;
}
...
function resetNums(){ //resets the number on screen clearing arrays, and display string, and iterator for arrays.
    ...
    displ="";
    i=0;
    document.getElementById("labeltop").style.color="transparent";
    document.getElementById("labelbott").innerText="Enter Numbers.";
}
function changeColor(){
    document.getElementById("labeltop").style.color="rgb(207, 207, 207)"; //Used for repetitive usage.
}
function reciprocal(){
    if(currNum!=""){
        changeColor();
        document.getElementById("labeltop").innerText=displ;
